...
class GameHelper {
  ...
  static showDamageText(
    scene: Scene,
    x: number,
    y: number,
    text: string,
    criticalAttack: boolean,
    textSyle?: Partial<Phaser.Types.GameObjects.Text.TextStyle>,
  ) {
  ...
  static getRealDamage(scene: Scene, actor: Actor, target: Actor, options: RealDamageOptions): GetRealDamageState {
    ...
    if (actor.status.alive && target.status.alive) {
      ...
      this.showDamageText(scene, target.x, target.y, initalState.damage.toString(), initalState.isCritical);
    }
    return initalState;
  }
}
